...
function topK(nums, k) {
    ...
    for (var _i = 0, nums_1 = nums; _i < nums_1.length; _i++) {
        var num = nums_1[_i];
        if (freqK[num] === undefined)
            freqK[num] = 1;
        else
            freqK[num]++;
    }
    var sorted = Object.entries(freqK).sort(function (a, b) { return b[1] - a[1]; });
    console.log({ sorted: sorted });
    return sorted.slice(0, k).map(function (_a) {
        var key = _a[0];
        return Number(key);
    });
}
console.log(topK([1, 2, 3, 4, 4, 4, 3], 2));
